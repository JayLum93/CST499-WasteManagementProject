...
local products = require ("productList")

products ={
    KrogerPurifiedDrinkingWater = "Recycling Bin",
...
function networkListener( event )
    ...
    if ( event.isError ) then
       ...
       print (data.name)
       prodObj = display.newText("Product Name:\n"..data.name, 0,-150,189, 0, native.systemFont, 18)
       prodObj:setFillColor( 0, 0, 0 )
       sScreen:insert(prodObj)
    end

    str = string.gsub(data.name, "%s+", "")
    print (str)
    for key,value in pairs(products) do
      if (key == str) then
        print (value)
        valObj = display.newText("Please place in "..value.."!",0,-100,native.systemFont,18)
        valObj:setFillColor(0,0,0)
        sScreen:insert(valObj)
        end
end
    return true
end
...
function codeListener(message)
    -- message variable contains the value of a scanned QR Code or a barcode.
        ...
        local myText = display.newText("Barcode: "..strBarcode, 0,-200, native.systemFont, 18 )
        myText:setFillColor( 0, 0, 0 )
        sScreen:insert(myText)
        network.request( "https://api.outpan.com/v2/products/"..strBarcode.."?apikey=af5af3064e3fdd59d3356f0bea534974", "GET", networkListener )
        return true
end
...
--network.request( "https://api.outpan.com/v2/products/"..name.."?apikey=af5af3064e3fdd59d3356f0bea534974", "GET", networkListener )
